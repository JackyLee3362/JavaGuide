...
![why-redis-so-fast](assets/why-redis-so-fast.png)
...
> [!TIP]
>
> 旁路缓存（写时先更新 db 再删除 cache）、读写穿透、异步读写
...
![Redis 发布订阅 (pub/sub) 功能](assets/redis-pub-sub.png)
...
![pub/sub 实现消息队列演示](assets/redis-pubsub-message-queue.png)
...
- 对于范围查询来说，平衡树也可以通过中序遍历的方式达到和跳表一样的效果
- 但是它的每一次插入或者删除操作都需要保证整颗树左右节点的绝对平衡，只要不平衡就要通过旋转操作来保持平衡，
- 这个过程是比较耗时的。跳表诞生的初衷就是为了克服平衡树的一些缺点
